    ...
    def calculate(self, i, j):
        ...
        for a in range(0, len(primitive_gaussian_array_i)):
            for b in range(0, len(primitive_gaussian_array_j)):
                ...
                l_2 = primitive_gaussian_array_j[b].get_integral_exponents()
                ...
                for c in range(0, len(primitive_gaussian_array_k)):
                    ...
                    s_ij += n_1 * n_2 * c_1 * c_2 * c_3 * OverlapIntegral().primitive_overlap_integral(primitive_gaussian_array_i[a], primitive_gaussian_array_k[c], r_a, r_b)
                t_ij += s_ij
        return t_ij
