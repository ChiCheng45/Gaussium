...
class Energy:
    ...
    def calculate_energy(self, nuclei_array, basis_set, symmetry_object):
        ...
        if self.method == 'RHF':
            electron_energy, correlation = RestrictedHF(
                nuclei_array, basis_set, self.electrons, symmetry_object, self.processors
            ).energies()

        if self.method == 'UHF':
            electron_energy, correlation = UnrestrictedHF(
                nuclei_array, basis_set, self.electrons, self.multiplicity, symmetry_object, self.processors
            ).energies()
        ...
        if self.method == 'MP2':
            electron_energy, correlation = MoellerPlesset(
                RestrictedHF(nuclei_array, basis_set, self.electrons, symmetry_object, self.processors)
            ).energies()

        if self.method[0] == 'DFT':
            electron_energy, correlation = RestrictedKohnSham(
                nuclei_array, basis_set, self.electrons, symmetry_object, self.processors, self.method[1],
                self.method[2]
            ).energies()
