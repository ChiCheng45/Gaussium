from src.main.common import BinomialCoefficientsFunction, Vector, BoysFunction
...
class ElectronRepulsionIntegral:
    ...
    def sigma_function(l, l_1, l_2, a, b, r, g):
        out = BinomialCoefficientsFunction.calculate_coefficient(l, l_1, l_2, a, b) * ((fac(l) * g**(r - l)) / (fac(r) * fac(l - 2*r)))
        return out
    ...
    def gaussian_product_factor(a_1, a_2, a_3, a_4, a_p, a_q, r_a, r_b, r_c, r_d):
        ...
        out = ((2 * pi**2) / (a_p * a_q)) * sqrt(pi / (a_p + a_q)) * exp(- ((a_1 * a_2 * r_ab**2) / a_p) - ((a_3 * a_4 * r_cd**2) / a_q))
        return out
