...
class ObaraSaika:
    ...
    @staticmethod
    def os_recursive(g, xyz, m):
        begin = ObaraSaika.os_begin
        count = ObaraSaika.os_count
        ...
        out1 = (r_5[xyz] - r_1[xyz]) * begin(g[0], g[1], g[2], g[3], m)
        out2 = (r_7[xyz] - r_5[xyz]) * begin(g[0], g[1], g[2], g[3], (m+1))
        if g[4].integral_exponents[xyz] >= 0:
            out3 = count(g[0].integral_exponents[xyz]) * (1 / (2 * a_5)) * begin(g[4], g[1], g[2], g[3], m)
            out4 = - count(g[0].integral_exponents[xyz]) * (rho / (2 * a_5**2)) * begin(g[4], g[1], g[2], g[3], (m+1))
        else:
            out3 = 0
            out4 = 0
        if g[5].integral_exponents[xyz] >= 0:
            out5 = count(g[1].integral_exponents[xyz]) * (1 / (2 * a_5)) * begin(g[0], g[5], g[2], g[3], m)
            out6 = - count(g[1].integral_exponents[xyz]) * (rho / (2 * a_5**2)) * begin(g[0], g[5], g[2], g[3], (m+1))
        else:
            out5 = 0
            out6 = 0
        if g[6].integral_exponents[xyz] >= 0:
            out7 = count(g[2].integral_exponents[xyz]) * (1 / (2*(a_5 + a_6))) * begin(g[0], g[1], g[6], g[3], (m+1))
        else:
            out7 = 0
        if g[7].integral_exponents[xyz] >= 0:
            out8 = count(g[3].integral_exponents[xyz]) * (1 / (2*(a_5 + a_6))) * begin(g[0], g[1], g[2], g[7], (m+1))
        else:
            out8 = 0
