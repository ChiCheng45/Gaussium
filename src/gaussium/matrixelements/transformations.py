...
def molecular_orbitals(repulsion, coefficients):
    """Converts the two electron repulsion integrals from atomic
    orbital to the molecular orbital basis set.

    Parameters
    ----------
    repulsion : np.array
    coefficients : np.matrix
        This is the orbital coefficients matrix from the Hartree
        Fock equations.
...
def blocked_spin_basis_set(repulsion):
    """Converts the two electron repusion integrals to spatial orbitals
    to spin orbitals in the blocked form.
...
def spin_basis_set(repulsion):
    """Converts the two electron repusion integrals of spatial orbitals
     to spin orbitals.
...
def spin_orbital_energies(orbital_energies):
    """Converts the the orbital energies for spatial orbital to
    spin orbitals.
...
def spin_basis_anti_physicist(repulsion):
    """Converts the two electron repusion integrals of spatial orbitals
    to antisymmetrized two electron integrals of spin orbitals in
