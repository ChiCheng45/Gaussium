...
def read_basis_set_file(file_input_basis, nuclei_array):
    """Reads a GAMESS-UK formatted gaussian basis set file using a
    list of nuclei objects.
    ...
    See Also
    --------
    expand_basis_set : function for expanding L, P, D and higher
    functions
    ...
    for a in range(len(nuclei_array)):
        ...
        # basis set file data is now in a more convenient form for
        # expand_basis_set to process
...
def read_mol_file(file_input_mol):
    """Reads a basic xyz mol file and returns a list of nuclei objects,
    the electron count and multiplicity.
