...
class TestHartreeFock(TestCase):
    ...
    def setUp(self):
        ...
        hydrogen_s_1 = MagicMock(contraction=0.15432897, exponent=3.42525091, coordinates=(0.0000, 0.0000, 1.4632),
        integral_exponents=(0, 0, 0), normalisation=1.794441832218435)
        hydrogen_s_2 = MagicMock(contraction=0.53532814, exponent=0.62391373, coordinates=(0.0000, 0.0000, 1.4632),
        integral_exponents=(0, 0, 0), normalisation=0.5003264923314032)
        hydrogen_s_3 = MagicMock(contraction=0.44463454, exponent=0.16885540, coordinates=(0.0000, 0.0000, 1.4632),
        integral_exponents=(0, 0, 0), normalisation=0.18773545851092535)

        helium_s_1 = MagicMock(contraction=0.15432897, exponent=9.75393461, coordinates=(0.0000, 0.0000, 0.0000),
        integral_exponents=(0, 0, 0), normalisation=3.9336432656254527)
        helium_s_2 = MagicMock(contraction=0.53532814, exponent=1.77669115, coordinates=(0.0000, 0.0000, 0.0000),
        integral_exponents=(0, 0, 0), normalisation=1.0967787981767012)
        helium_s_3 = MagicMock(contraction=0.44463454, exponent=0.48084429, coordinates=(0.0000, 0.0000, 0.0000),
        integral_exponents=(0, 0, 0), normalisation=0.41154131374122654)

        hydrogen_basis = MagicMock(primitive_gaussian_array=[hydrogen_s_1, hydrogen_s_2, hydrogen_s_3],
        coordinates=(0.0000, 0.0000, 1.4632), integral_exponents=(0, 0, 0))
        helium_basis = MagicMock(primitive_gaussian_array=[helium_s_1, helium_s_2, helium_s_3],
        coordinates=(0.0000, 0.0000, 0.0000), integral_exponents=(0, 0, 0))

        point_group = MagicMock(rotation_symmetry=[], reflection_symmetry=[], improper_rotation=[],
        inversion_symmetry=[], label='C_{1}')
